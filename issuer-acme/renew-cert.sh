...
export CERT_NAME=$(kubectl get certificates -n cottage -o name | cut -d'/' -f 2)
export ORIGINAL_CERT_EXPIRY=$(kubectl get certificate -n cottage -o=jsonpath='{.items[0].status.notAfter}')
...
kubectl delete certificate $CERT_NAME -n cottage

export COTTAGE_CERTS_SECRET=$(kubectl get secrets -n cottage -o=jsonpath='{.items[0].metadata.name}')
kubectl delete secret $COTTAGE_CERTS_SECRET -n cottage
...
# Use date -d if you're using non-BSD/MacOS
kubectl describe certificates le-crt -n ambassador

export NEW_CERT_EXPIRY=$(kubectl get certificate -n cottage -o=jsonpath='{.items[0].status.notAfter}')
...
export SECERT_NAME=$(kubectl get secrets -n cottage tls-lets-secret -o name)
if [ "$SECERT_NAME" == "secret/tls-lets-secret" ];
then
